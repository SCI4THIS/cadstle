...
let utf8_c = function(p) {
  let len = 0;
  let i = 0;
  while (g.memory[p + len] != 0) {
    len++;
  }
  let bytes = g.memory.slice(p, p + len);
...
wasm3_proto["srand"] = "v(i)"
let srand = function (i32) {
  /* We will use Math.random(), which doesn't need a seed */
  if (DEBUG_FUNCTION_PRINTS) {
    console.log("srand(" + i32 + ")");
  }
  return;
}

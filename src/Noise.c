...
#if PATCH
const int Noise3DTexSize = 128;
const int Noise3DTexSize_X = Noise3DTexSize;
const int Noise3DTexSize_Y = Noise3DTexSize;
const int Noise3DTexSize_Z = 1;
GLubyte Noise3DTexPtr[Noise3DTexSize_X * Noise3DTexSize_Y * Noise3DTexSize_Z * 4];

unsigned char *get_Noise3DTexPtr()
{
...
void make3DNoiseTexture()
{
...
#ifndef PATCH
	Noise3DTexPtr = (GLubyte*) malloc(Noise3DTexSize_X * Noise3DTexSize_Y * Noise3DTexSize_Z * 4);
#endif
	for (f = 0, inc = 0; f < numOctaves; ++f, frequency *= 2, ++inc, amp *= 0.5)
	{
		...
		for (i = 0; i < Noise3DTexSize_X; ++i, ni[0] += inci)
		{
			incj = 1.0 / (Noise3DTexSize / frequency);
			for (j = 0; j < Noise3DTexSize_Y; ++j, ni[1] += incj)
			{
				inck = 1.0 / (Noise3DTexSize / frequency);
				for (k = 0; k < Noise3DTexSize_Z; ++k, ni[2] += inck, ptr += 4)
